...
for plot in plots:
    ...
    for values_i in splits:
        ...
        for creater in values:
            ...
            # put mean in ypoints and standard error in errors
            l_ypoints_ls[-1].append(np.mean(values[creater]))
            l_low_errors_ls[-1].append(np.mean(values[creater]) - np.std(values[creater], ddof=1) / np.sqrt(len(values[creater])))
            l_high_errors_ls[-1].append(np.mean(values[creater]) + np.std(values[creater], ddof=1) / np.sqrt(len(values[creater])))
    ...
    for xpoints_ls, ypoints_ls, tlabel, low_error, high_error in zip(l_xpoints_ls, l_ypoints_ls, labels, l_low_errors_ls, l_high_errors_ls):
        ...
        plt.fill_between(x, low_error, high_error, alpha=0.5)
