...
void prey::step(std::list<std::shared_ptr<predator>>& pred_vec) {
	...
	if (sdlen(pos + norm(dvec, prey_speed)) > 1) {
	...
	else if (len(dvec) < prey_see_range) { // move away from nearest predator, distance normalized to speed
		dvec = norm(dvec, prey_speed);
		pos = pos - dvec;
...
bool predator::step (std::list<std::shared_ptr<prey>>& prey_vec, std::list<std::shared_ptr<predator>>& pred_vec) {
	if (energy < -energy_on) return true; // means this creature dies in this frame
	if (energy > energy_on) {
		vec2f childpos = pos + polartocart((float)rand() / RAND_MAX * 2 * PI, 5*pred_speed); // spawn child a little aways so they are not practically the same entity
	...
	if (len(dvec) < pred_speed) {
		(*nearest) = std::shared_ptr<prey>(new prey());
	}

	else if (len(dvec) < pred_see_range) { // move in direction of nearest prey, distance normalized to speed
		dvec = norm(dvec, pred_speed);
		pos = pos + dvec;
		...
		phi_idle += (float)rand() / RAND_MAX * 2 * idle_dir_c - idle_dir_c;
		if (phi_idle < 0) phi_idle += 2 * PI;
		if (phi_idle > 2 * PI) phi_idle -= 2 * PI;
		pos += polartocart(phi_idle, pred_speed * idle_slow);
		if (sdlen(pos) > 1) {
			pos = norm(pos, 1);
